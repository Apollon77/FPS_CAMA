...
bool FPSUserAction(const Command_Packet::Commands::Commands_Enum command, const Response_Packet::ResultCodes::ResultCodes_Enum action)
{
  ...
  if (command == Command_Packet::Commands::Enroll) {
      Serial.println();
      switch(action) {
        case Response_Packet::ResultCodes::GD_NEED_RELEASE_FINGER: // Verify, Identify, Enroll
          ...
          break;
        case Response_Packet::ResultCodes::GD_NEED_FIRST_SWEEP: // Enroll
          Serial.println("Please scan your finger for enrollment: 1st Sweep.");
          break;
        case Response_Packet::ResultCodes::GD_NEED_SECOND_SWEEP: // Enroll
          Serial.println("Please scan your finger for enrollment: 2nd Sweep.");
          break;
        case Response_Packet::ResultCodes::GD_NEED_THIRD_SWEEP: // Enroll
          Serial.println("Please scan your finger for enrollment: 3rd Sweep.");
          break;
        default:
...
/*
    val2 = fps.clearAllTemplate();
    Serial.print("FPS - clearAllTemplate = ");
    Serial.println(val2);
    Serial.println();
    Serial.println();
*/
...
void Enroll()
{
    ...
    bool templateStatus = fps.getTemplateStatus(enrollId);
    if (templateStatus) {
        Serial.println("Verified, Template id is free.");
        Serial.println();
        Serial.println();
    }
    else {
    ...
    if (result) {
        Serial.println("Enrollment process completed successfully!");
